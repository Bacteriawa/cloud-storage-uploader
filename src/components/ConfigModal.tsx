...
export default function ConfigModal({ isOpen, onClose, onSave }: Props) {
  ...
  const [step, setStep] = useState(1); // 1: password, 2: config
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const existing = loadConfig(password);
    if (existing) {
      setConfig(existing);
    }
    setStep(2);
  ...
  const handleConfigSubmit = (e: React.FormEvent) => {
    ...
    saveConfig(finalConfig, password);
    onSave(finalConfig);
    onClose();
  };
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ padding: '10px', background: 'rgba(99, 102, 241, 0.1)', borderRadius: '12px', color: 'var(--accent)' }}>
                  {step === 1 ? <Lock size={24} /> : <Settings size={24} />}
                </div>
                <h2 style={{ fontSize: '20px', fontWeight: 600 }}>
                  {step === 1 ? t('unlockConfig') : t('r2Settings')}
                </h2>
              </div>
            ...
            {step === 1 ? (
              <form onSubmit={handlePasswordSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: '8px', fontSize: '14px', color: 'var(--text-secondary)' }}>{t('masterPassword')}</label>
                  <input
                    type="password"
                    required
                    value={password}
